...
export const MyGeneratedILAWView: React.FC<MyGeneratedILAWViewProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'matrix' | 'four_day' | 'las' | 'slides' | 'answers' | 'checker' | 'poster' | 'science' | 'writing'>('four_day');

  const { header, activitySheets, presentationSlides } = plan;

  const [autoSyncToDrive, setAutoSyncToDrive] = useState<boolean>(() => {
    const saved = localStorage.getItem('boiser_auto_sync_to_drive');
  ...
  const handleToggleAutoSync = (enabled: boolean) => {
    ...
    localStorage.setItem('boiser_auto_sync_to_drive', String(enabled));
  };

  const executePushToGoogleDrive = async (silent = false) => {
    setIsSyncingToDrive(true);
    try {
      const result = await pushILAWDocxToGoogleDrive(plan);
      if (result.success && result.file) {
        setDriveSyncResult({
          success: true,
          message: `✓ Saved .docx to Google Drive (${BOISER_LESSONS_FOLDER_NAME})`,
        ...
        if (!silent) {
          setDriveSyncResult({
            success: false,
            message: result.error || 'Failed to sync with Google Drive. Check authentication.'
          });
        }
      }
    } catch (err: any) {
  ...
  // Auto-sync when plan is rendered if autoSyncToDrive is on
  useEffect(() => {
    if (autoSyncToDrive && plan && plan.header && plan.header.lesson) {
      executePushToGoogleDrive(true);
    }
  }, [plan.header.lesson, plan.header.term, autoSyncToDrive]);
      ...
      <div className="bg-[#002776] text-white rounded-3xl p-6 sm:p-8 shadow-md border-b-4 border-[#FCD116] space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => executePushToGoogleDrive(false)}
              disabled={isSyncingToDrive}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-400 hover:to-emerald-500 text-white text-xs font-bold transition shadow-sm cursor-pointer disabled:opacity-50"
              title="Push .docx directly to 'Boiser-Generated-Lessons' in Google Drive"
            >
              {isSyncingToDrive ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CloudUpload className="w-3.5 h-3.5" />}
              <span>{isSyncingToDrive ? 'Syncing to Drive...' : 'Push .docx to Drive'}</span>
            </button>
          ...
          {/* 🔄 AUTOMATIC SYNC TO DRIVE TOGGLE */}
          <div className="flex items-center gap-2 bg-black/25 px-3 py-1.5 rounded-2xl border border-white/20 backdrop-blur-xs">
            <Cloud className="w-4 h-4 text-emerald-400" />
            <div className="flex flex-col text-left">
              <span className="text-[11px] font-bold text-white flex items-center gap-1">
                Auto-Sync to Google Drive
                <span className="text-[9px] px-1.5 py-0.2 bg-emerald-400/30 text-emerald-300 rounded font-mono">
                  /{BOISER_LESSONS_FOLDER_NAME}
                </span>
              </span>
              <span className="text-[9px] text-blue-200">
                Pushes .docx on plan generate
              </span>
            </div>
        ...
        {/* Live Drive sync feedback notification */}
        {driveSyncResult && (
          <div
            className={`p-3 rounded-2xl text-xs font-medium flex items-center justify-between animate-fade-in ${
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span>{driveSyncResult.message}</span>
            </div>
            {driveSyncResult.webViewLink && (
              <a
                href={driveSyncResult.webViewLink}
                target="_blank"
                rel="noreferrer"
                className="px-2.5 py-1 bg-white/20 hover:bg-white/30 text-white rounded-lg font-bold flex items-center gap-1 text-[11px] transition cursor-pointer"
              >
