...
import { SciencePPTGenerator } from './components/SciencePPTGenerator';
import { StudentGradingApp } from './components/StudentGradingApp';
...
const OWNER_EMAIL = 'boisersteavenkinth@gmail.com';
const OWNER_NAME = 'Steaven Kinth D. Boiser';
...
export default function App() {
  ...
  const handleUnlock = (masterAccess: boolean) => {
    // Only the true owner can unlock Master Mode
    const actuallyOwner = (user?.email === OWNER_EMAIL) || (masterAccess && user?.email === OWNER_EMAIL);
    
    setIsMasterMode(actuallyOwner);
    setIsAppUnlocked(true);
    if (actuallyOwner) {
      triggerAlert(`✓ IDENTITY VERIFIED: Welcome back, ${OWNER_NAME}. Master Coding Access Unlocked.`);
    } else if (masterAccess) {
      // If they passed biometric but aren't the email-matched owner (impossible in this context but good for safety)
      triggerAlert('⚠️ Biometric mismatch with Owner account. Restricted access applied.');
      setIsMasterMode(false);
    }
  };

  // Shielding & Startup Check
  if (!isAppUnlocked) {
...
Search Query: "${q}" | Verified against Google Scholar & DepEd Research Portal.`);
  ...
  const navPages = [
    ...
    { id: 'curriculum', label: '📚 Curriculum DB' },
    { id: 'math', label: '🧮 Math' },
  ...
  return (
    <SecurityShield isOwner={isOwner} ownerName={OWNER_NAME}>
      ...
      {/* 1. STICKY TOP HEADER */}
      <header className="bg-gradient-to-r from-[#092b62] to-[#0b67b2] text-white p-5 sticky top-0 z-50 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="margin-0 text-xl sm:text-2xl font-black tracking-tight flex items-center gap-2">
              ⚡ BOISER POWER TOOLS — ULTIMATE
            </h1>
            <p className="text-xs sm:text-sm text-blue-100/90 mt-1 font-medium">
              Official DepEd K–12 Bulk Competency Engine &amp; Integrated ILAW Generator
            </p>
          </div>
          <div className="flex items-center gap-2">
            <span className={`text-xs font-bold px-3 py-1 rounded-full border flex items-center gap-1.5 shadow-2xs ${
              ...
              <span>{isOnline ? 'Connected. Data is up to date.' : `Offline Mode (Cached ${offlineTimestamp})`}</span>
            </span>
          </div>
        </div>
      </header>
                ...
                <table className="w-full">
                  <thead>
                    <tr>
                      ...
                      <th className="font-bold">Subject</th>
                  ...
                  <tbody>
                    {getFilteredDatabaseList().map((x) => (
                      <tr key={x.id} className="hover:bg-stone-50/50">
                        <td className="font-mono font-bold text-blue-900">{x.code}</td>
                        <td>{x.level}</td>
                        <td>{x.grade}</td>
                        <td className="font-bold">{x.subject}</td>
                        <td><span className="font-bold text-amber-800">T{x.term}</span></td>
                        <td className="font-serif leading-relaxed pr-4">"{x.competency}"</td>
                        <td>
                          <div className="font-bold text-stone-500">{x.source}</div>
        ...
        {activeTab === 'writing' && (
          <div className="bg-white border border-[#dce3ee] rounded-3xl p-5 sm:p-7 shadow-sm space-y-4">
            ...
            <textarea
              ...
              placeholder="Paste student text block here..."
              className="bg-stone-50 text-xs"
            />
            <div className="flex gap-2">
              <button onClick={handleCheckWriting} className="px-5 py-2.5 bg-[#0b4ea2] text-white rounded-xl text-xs font-bold cursor-pointer">
                Check Text
              </button>
              <button onClick={handleAiScreen} className="px-5 py-2.5 bg-[#b45309] text-white rounded-xl text-xs font-bold cursor-pointer">
                AI-Writing Screening
              </button>
            </div>
            {writingOutput && <pre className="out text-xs font-mono">{writingOutput}</pre>}
          </div>
        )}
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && chatInput.trim()) {
                    if (!isOnline) {
                      triggerAlert('You are offline. Message queued to send when reconnected.');
                    }
                    setChatMessages([...chatMessages, { sender: 'Steaven Kinth Boiser (Admin)', text: chatInput, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), verified: true }]);
                    setChatInput('');
                    triggerAlert('✓ Fact-checked message sent to Google Chat space!');
                  }
                }}
                placeholder={isOnline ? "Type a verified message to Google Chat space..." : "Offline: message will be queued..."}
      ...
      {/* 5. FOOTER */}
      <footer className="bg-white border-t border-[#dce3ee] py-6 text-xs text-stone-500">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
          <div className="flex items-center gap-2 justify-center">
            <span className="font-bold text-stone-800">⚡ BOISER POWER TOOLS</span>
            <span>•</span>
            <span>Ultimate Teacher &amp; Student Workspace</span>
          </div>
          <span className="text-[11px] text-stone-400">
            Powered by modern React client-side computation &amp; localized cache storage.
          </span>
        </div>
      </footer>
    </div>
